...
const childrenSchoolWay = async (userId: number): Promise<ChildrenSchoolWay[] | null> => {
  const accessToken = localStorage.getItem('ACCESS_TOKEN');
  try {
    const response = await axios.get(
      `${API_BASE_URL}api/parents/schoolway?userId=${userId}`,
      {
        headers: {
...
export default childrenSchoolWay;
